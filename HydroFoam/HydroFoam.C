...
#include "CorrectHydroBoundaryConditions.H"
...
#   include "createTimeControls.H"
  ...
  runTime.controlDict().lookup("writeIntervalHydro") >> Steps;
  Foam::label Hydrocounter = 0;

ofstream FilePostVelocity;
FilePostVelocity.open("FilePostVel.txt",std::ios::app);
 FilePostVelocity << "t" << "\t" << "Uxmax" << "\t" << "Uymax" << "\t" << "max(magU)" << "\n";
...
// * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * //
    ...
    dimensionedScalar mu("mu",Foam::dimensionSet(1,-1,-1,0,0,0,0),scalar(0.0));
...
#       include "readTimeControls.H"
	  ...
	  Hydrocounter++;
		...
		if(Hydrocounter/Steps==label(1))
		  {
	    FilePostVelocity << runTime.value() << "\t" << Foam::max(mag(U.component(0))).value() << "\t" << 
		  Foam::max(mag(U.component(1))).value() << "\t" << Foam::max(mag(U)).value() << "\n";
		Hydrocounter=0;
		  }
      ...
      //    dbnsFlux.EvaluateLimiter();
	    ...
	    tmp<volScalarField> tEn = Cv*T + 0.5*magSqr(U);
	    tEn().boundaryField() = Cv.value()*T.boundaryField();
	    tmp<volScalarField> tLapEn = fvc::laplacian(mu,tEn());
