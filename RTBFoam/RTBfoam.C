...
#   include "setRootCase.H"
  ...
  std::string postProcFname = args.caseName();
  postProcFname+=".csv";
...
FilePostVelocity << "t" << "\t" << "Uxmax" << "\t" << "Uymax" << "\n";
*/
  // Output and post-processing:
  Foam::label Steps = 0;
  runTime.controlDict().lookup("writePostTB") >> Steps;
  Foam::label Postcounter = 0;
  
  ofstream postProcTB; // Passing the filename as argument
  postProcTB.open(postProcFname,std::ios::out); // Replacing the content
  
  postProcTB << "t"<<","<<"wMax"<<","<<"thetaMax"<<"\n";
  
  scalar thetamax1 = max(thetaP).value();
  scalar thetamax2 = max(thetaP.boundaryField());
  scalar thetamax = max(thetamax1,thetamax2);

    postProcTB << runTime.value() << "," << Foam::max(mag(U.component(1))).value() <<
              "," << thetamax  << "\n";
...
// * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * //
    ...
    //dimensionedScalar mu("mu",Foam::dimensionSet(1,-1,-1,0,0,0,0),mu_scalar);
